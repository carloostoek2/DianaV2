...
- **Non-sensitive images are captioned by Gemini** (`llm/gemini_vision.py`, model `gemini-2.5-flash`, key `GEMINI_API_KEY`): short Spanish-neutral caption, 40 words max. Provider failure is fail-open — the plain `[imagen]` tag is kept and the turn never breaks.
