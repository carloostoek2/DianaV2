...
### Follow-ups worth a small hardening item

| Residual | Priority | Origin |
|----------|----------|--------|
| `is_blocked` / claimed approvals not visible to recontact | medium | Pool2 |
| Decider/process **reload thresholds** after calibration (today: next boot) | medium | Pool3 |
| MetricsJob durable last-success week marker (in-memory v1) | low | Pool3 |
...
| Auto-`schedule_recontact` on every VIP message (BR-07 is cancel-only) | Pool2 |
| Promo hard rate-limit silence | Pool2 / CLARIFY |
| Owner false-positive escalation mark model (FP rate stays 0.0) | Pool3 |
...
4. **Recontact:** `FEATURE_RECONTACT_ENABLED=true`; verify freeze/pause/`is_blocked`, BR-07 cancel, job cadence; watch claimed-approval residual.
...
6. **Calibration last:** `FEATURE_CALIBRATION_ENABLED=true` after enough traces exist; verify margin 0.05, smooth, owner drift alerts; **restart** (or future reload) so Decider picks new thresholds.
...
> **Fase 3 implementation (Pools 1–3) complete.** All F3 feature flags remain **default false**. Next: gradual ops enablement + residual hardening as needed.
