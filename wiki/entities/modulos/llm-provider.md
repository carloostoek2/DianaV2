...
Toggle de thinking (landed 2026-08-09, omitido en el inventario del 11): `DeepSeekProvider(thinking_enabled=...)` cableado a `settings.llm_thinking_enabled` (default `True`). El esfuerzo de razonamiento se configura con `settings.llm_thinking_effort` (default `"medium"`, env `LLM_THINKING_EFFORT`, valores low/medium/high).

- `generate` (borradores de texto libre): con thinking on envía `"thinking": {"type": "enabled"}` y `"reasoning_effort": <llm_thinking_effort>`; timeout mínimo 120 s y `max_tokens` default 4096 (si no, 1024). Si CoT agota el presupuesto (`content` vacío + `finish_reason=length` + hubo `reasoning_content`), reintenta una vez con la misma config y, si falla igual, un tercer intento con thinking **OFF**. Callers con `max_tokens < 512` (p. ej. recontacto) fuerzan thinking off (sin effort ni ladder). Nunca usa `reasoning_content` como draft.
