...
```json
{
  ...
  "properties": {
    ...
    "doctrina": {
      "type": "number", "minimum": 0, "maximum": 1,
      "description": "¿El borrador cumple las reglas de negocio REALES del fence <<KNOWLEDGE_POLICY_DATA>>? Memoria y perfil no puntúan doctrina (sirven a precisión/consistencia). Tres casos: (1) hay bloque 'knowledge.policy' → medir cumplimiento real solo contra esas reglas (puede variar en todo [0,1]); (2) sin policy y 'comprension.needs_policy=false' → NO APLICA: neutro honesto 0.5, nunca un 'neutral-alto ~0.7' que parezca cumplimiento medido; (3) sin policy y 'needs_policy=true' → regla requerida y ausente: ~0.2 (señal de doctrina faltante; la zona gris la gestiona el Decisor). Un valor 'no aplica' no debe bloquear la autonomía del turno (ver B.8)."
    },
    "consistencia": {
      "type": "number", "minimum": 0, "maximum": 1,
...
- **Doctrina "relevante"** (`is_doctrine_relevant`): doctrina solo se compara contra `doctrine_min` cuando el turno trajo una regla real O la requirió (`needs_policy`). Un turno sin regla y sin necesidad tiene "nada que cumplir": su valor de doctrina no debe bloquear la autonomía ni reportarse como dimensión bloqueante (shadow) ni inflar la severidad de una corrección. El número interno "no aplica" (0.5) es informativo, no un gate. En la UI de la dueña (borrador y modo sombra) se muestra `doc=no aplica` / `Doctrina: no aplica` (sin ❌); el ~0.2 de regla faltante **sí** se muestra como número. Nunca inferir "no aplica" porque el valor sea 0.5 (un cumplimiento real o un dummy de zona gris puede coincidir). Residual: si se enciende `FEATURE_CALIBRATION_ENABLED`, los 0.5 de N/A contaminan percentiles de `doctrine_min` — hay que filtrar no-relevante **antes** de calibrar (no implementado).
